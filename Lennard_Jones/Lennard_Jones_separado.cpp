...
Archivo::Archivo(const string& nombre_, bool habilitado_) : habilitado(habilitado_)
{
    if (habilitado)
    {
        archivo.open(nombre);
...
datos("Datos/Lennard_Jones_" + to_string(N_) + "_" + to_string(iteraciones_) + "_" + to_string((int)L_) + ".csv", save_to_file),
velocidades("Datos/Velocidades_" + to_string(N) + "_" + to_string(iteraciones) + "_" + to_string((int)L) + ".csv", save_to_file),
energias("Datos/Energias_" + to_string(N) + "_" + to_string(iteraciones) + "_" + to_string((int)L) + ".csv", save_to_file)
{   
    ...
    Archivo parametros("Parametros.txt", save_to_file);
...
void Simulador::integrar()
{
    ...
    for (int i = 0; i < N; i++)
    {
        posicion_inicial[0][i] = particulas[i].get_xy()[0];
        posicion_inicial[1][i] = particulas[i].get_xy()[1];
    }
...
int main(){
    ...
    Simulador simulacion(N, pasos, 0.002, L, 1, vmax);
