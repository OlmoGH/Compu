...
int main(){
    ...
    Simulador simulacion(N, iteraciones, paso, L, vmax, true, n_threads);

    // Inicializamos las partículas
    simulacion.inicializar(Modo::Cuadricula);
    ...
    for (int n = 1; n < iteraciones; n++)
    {
        if (n == incremento)
        {
            for (int i = 0; i < N; i++)
            {
                ...
                simulacion.particulas[i].set_vxvy(1.5 * v[0], 1.5 * v[1]);
            }
